...
- col.keep:
  ...
  Names of additional columns to include in the result (e.g. exposure
  and covariate variables)

- ages:

  Vector of ages (years) to create age-at-risk strata. Any participant
  follow-up outside these ages is dropped. (Must be a numeric vector
  with distinct, increasing values. Default: c(0, 100))

- age.labels:

  Vector of name for age-at-risk strata (Default:
  `paste0(ages[-length(ages)], "-", ages[-1])`)

- null.status:

  Value to use in status column for intervals before the final interval
  (Default: 0)

## Value

Data frame expanded by age-at-risk
